...
class STARiouEvaluator:
    def __init__(
        ...
        """
        :param star_path: path to VidSTG annotations
    ...
    def evaluate(self, predictions: List[Dict], video_predictions: List[Dict]):
        ...
        for video_id, video_pred in video_predictions.items():
            ...
            question_id = video_pred["question_id"]
...
class STAREvaluator(object):
    def __init__(
        ...
        """
        :param star_path: path to STAR annotations
        :param subset: train, val or test
        :param verbose: whether to print more information or not
        :param iou_thresholds: IoU thresholds for the vIoU metrics
        :param fps: number of frames per second
    ...
    def summarize(self):
        ...
        categories = set(x["question_id"].split("_")[:-1] for x in self.results.values())
        ...
        for x in self.results.values():  # sum results
            ...
            metrics[question_id]["gt_viou"] += x["gt_viou"]
            for thresh in self.iou_thresholds:
                if self.tmp_loc:
                    metrics[question_id][f"viou@{thresh}"] += x[f"viou@{thresh}"]
                metrics[question_id][f"gt_viou@{thresh}"] += x[f"gt_viou@{thresh}"]
